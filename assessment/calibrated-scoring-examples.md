# Calibrated Scoring Examples

Use these to calibrate your self-assessment before finalizing scores. Each example shows what each maturity level looks like across the 4D model for three function tracks.

**Find your track using the title mapping below, then go to that section.**

| Your title likely includes... | Use this track |
|-------------------------------|---------------|
| Content, campaigns, demand gen, brand, comms, marketing ops, growth | **Marketing — Content and Campaign Role** |
| Account executive, AE, SDR, BDR, sales ops, RevOps, revenue operations, sales enablement | **Sales and RevOps — Account Executive or RevOps Analyst Role** |
| Support, customer success, CSM, onboarding, implementation, technical account manager | **Support and Customer Success — Support Lead or CSM Role** |

If your role spans two tracks (e.g., you run RevOps but also own demand gen), pick the one that represents the majority of your recurring AI work.

---

## Marketing — Content and Campaign Role

### Delegation

**1 (Emerging)**
Asked AI to write a blog post headline. No context provided. Used output unchanged. No repeatable process.

**2 (Developing)**
Uses AI weekly to brainstorm campaign names and subject lines. Sometimes includes brand guidelines as context. Results are inconsistent — some outputs are reused, most are rewritten.

**3 (Capable)**
Has a documented workflow for writing email sequences: role context → audience → tone brief → draft → review checklist → approved/revised. Runs this on every campaign. Can show prompt library with 5+ reusable patterns.

### Description

**1 (Emerging)**
Prompt: "Write a LinkedIn post about our product launch." No further context. Accepts first output.

**2 (Developing)**
Prompt includes audience type and a few product details. Revises once when tone is off, but revisions are ad hoc.

**3 (Capable)**
Prompt structure: [Role context] + [Audience] + [Goal] + [Constraints] + [Format]. Maintains a template library. Can show multiple before/after examples demonstrating targeted iteration.

### Discernment

**1 (Emerging)**
Checks outputs for obvious errors. No formal checklist. Publishes AI content with minimal review.

**2 (Developing)**
Reviews for factual accuracy and brand voice before publishing. Occasionally checks for hallucinated claims. No documented criteria.

**3 (Capable)**
Runs a defined review pass before publishing: factual accuracy → brand voice → audience fit → compliance flag check. Keeps a decision log of what was kept, edited, or discarded with rationale.

### Diligence

**1 (Emerging)**
No disclosure practice. No awareness of which outputs have AI involvement.

**2 (Developing)**
Adds "AI-assisted" note in some internal documents. Inconsistent across work types.

**3 (Capable)**
Applies consistent disclosure in internal handoffs. Runs a privacy check before using any customer data in prompts. Named as review owner for published content.

---

## Sales and RevOps — Account Executive or RevOps Analyst Role

### Delegation

**1 (Emerging)**
Occasionally uses AI to draft follow-up emails. No structured approach. Does not reuse outputs.

**2 (Developing)**
Uses AI for pre-call research summaries and follow-up drafts. Workflow exists informally — sometimes skips it when busy.

**3 (Capable)**
Has a defined pre-call workflow: ICP brief → discovery question bank → objection prep → call debrief template. Runs it on every qualified opportunity. Can show a before/after from a closed deal.

### Description

**1 (Emerging)**
Prompt: "Write a follow-up email for a prospect." No company or deal context.

**2 (Developing)**
Includes company name and product use case. Revises for tone but rarely for accuracy or specificity.

**3 (Capable)**
Prompt includes: deal stage, stakeholder role, specific objection raised, desired next step, and tone guidance. Shows multiple iterations with rationale for each revision.

### Discernment

**1 (Emerging)**
Sends AI-drafted emails with a quick read-through only. No structured check.

**2 (Developing)**
Checks for factual accuracy and name/company correctness before sending. Rarely documents errors caught.

**3 (Capable)**
Review pass: factual accuracy → deal-context fit → tone → compliance. Documents cases where AI output missed deal context or introduced incorrect claims.

### Diligence

**1 (Emerging)**
No disclosure. No awareness of what customer data is included in prompts.

**2 (Developing)**
Avoids including raw customer data in prompts. Not consistent about reviewing for sensitive information.

**3 (Capable)**
Reviews prompts for customer data before sending to AI tool. Named as owner for all outbound reviewed and sent. Documents process in deal notes.

---

## Support and Customer Success — Support Lead or CSM Role

### Delegation

**1 (Emerging)**
Uses AI to draft canned responses for common tickets. No process. Reuses outputs inconsistently.

**2 (Developing)**
Has an informal triage workflow: copy ticket text → ask AI for suggested response → revise. Works for low-complexity tickets. Skips AI for anything sensitive.

**3 (Capable)**
Documented a tiered triage workflow: complexity check → AI-assisted draft → review against escalation criteria → approve or escalate. Runs on all Tier 1 tickets. Can show ticket volume and handle time before/after.

### Description

**1 (Emerging)**
Prompt: "Help me respond to an angry customer." No ticket context.

**2 (Developing)**
Includes ticket text and customer tier. Revises once if tone is off. Does not reuse the pattern.

**3 (Capable)**
Prompt structure: [Customer issue] + [Context: tier, history] + [Policy constraints] + [Tone: empathetic, direct] + [Format: response template]. Has 3+ reusable patterns by ticket type.

### Discernment

**1 (Emerging)**
Light review before sending. No criteria for what makes a good response.

**2 (Developing)**
Checks for accuracy and empathy. Flags when AI misses customer history or makes incorrect policy claims.

**3 (Capable)**
Review checklist: accuracy → policy compliance → escalation trigger check → tone. Documents cases where AI output was incorrect or insufficient with rationale.

### Diligence

**1 (Emerging)**
No disclosure. Includes customer account details in prompts without reviewing for sensitivity.

**2 (Developing)**
Aware of data sensitivity. Removes names from prompts inconsistently.

**3 (Capable)**
Reviews all prompts to remove PII before sending to AI tools. Named as accountability owner for all AI-assisted responses. Documents customer data handling in response notes.
